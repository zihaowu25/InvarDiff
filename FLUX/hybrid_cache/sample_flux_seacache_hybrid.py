...
def scheduler_ab(scheduler, step_idx: int) -> Tuple[float, float]:
...
def apply_sea_filter(
    ...
    a, b = scheduler_ab(scheduler, step_idx)
    gain = None
    for axis in dims:
        frequency = torch.fft.fftfreq(
            x32.shape[axis], device=x32.device, dtype=torch.float32
        )
        radius = frequency.abs()
        signal_power = 1.0 / (radius.pow(power_exp) + 1e-16)
        axis_gain = (
            a * signal_power
            / (a * a * signal_power + b * b + 1e-16)
        )
        shape = [1] * x32.ndim
    ...
    if norm_mode != "mean":
        raise ValueError("The official FLUX SeaCache path requires mean normalization")
    mean_gain = gain.mean()
...
def relative_l1(a: torch.Tensor, b: torch.Tensor) -> float:
    numerator = (a - b).abs().mean()
    denominator = b.abs().mean() + 1e-16
...
class SeaCachePolicy:
    ...
    def decide(
        ...
        if (
            step_idx == 0
            or step_idx == self.num_steps - 1
            or self.previous_modulated_input is None
        ):
            should_skip = False
            self.accumulated_distance = 0.0
            comparison_input = modulated_input
        else:
            grid_h = int(img_ids[:, 1].max().item() + 1)
            grid_w = int(img_ids[:, 2].max().item() + 1)
            comparison_input = modulated_input.reshape(
                modulated_input.shape[0],
                grid_h,
                grid_w,
                modulated_input.shape[-1],
            )
            comparison_input = apply_sea_filter(
                comparison_input,
                self.scheduler,
                step_idx,
                power_exp=2.0,
            ...
            self.accumulated_distance += relative_l1(
                comparison_input, self.previous_modulated_input
            )
            should_skip = self.accumulated_distance < self.threshold
            if not should_skip:
                self.accumulated_distance = 0.0
        self.previous_modulated_input = comparison_input
        return should_skip
...
class CalibrationRun:
    def __init__(
        ...
        self.step_mask = [False] * num_steps
        self.scores = {
            **{
    ...
    def callback(
        ...
        self.step_mask[step_idx] = bool(would_skip)
...
class HybridFluxTransformer(nn.Module):
    def __init__(self, base_model: nn.Module, num_steps: int):
        ...
        self.step_policy: Optional[SeaCachePolicy] = None
        self.reset_runtime()
    ...
    def forward(
        ...
        modulated_input = self.transformer_blocks[0].norm1(
            hidden_states, emb=temb
        )[0]
        if self.forced_observer_mask is not None:
            ...
            would_skip = self.step_policy.decide(
                self.step_idx, modulated_input, img_ids
            )
        else:
            would_skip = False
        actual_skip = (
...
def calibrate(pipe, model, prompts, args):
    print("Calibration pass 1/2: raw layer scores + SeaCache observer")
    raw_runs = []
    ...
    for prompt in tqdm(
        ...
        model.step_policy = SeaCachePolicy(
            pipe.scheduler,
            args.num_inference_steps,
            args.seacache_thresh,
        )
        raw_runs.append(run_calibration_prompt(pipe, model, prompt, args))
    ...
    provisional_step_books = [run.step_mask for run in raw_runs]
    ...
    torch.cuda.reset_peak_memory_stats()
    start = time.perf_counter()
    for prompt, step_mask in tqdm(
        zip(prompts, provisional_step_books),
        total=len(prompts),
        desc="Correction calibration",
        disable=args.disable_progress_bar,
    ):
        effective_books = apply_step_first(provisional_books, step_mask)
    ...
    observer_hit_rates = [
        sum(mask) / len(mask) for mask in provisional_step_books
    ]
    return final_books, observer_hit_rates
...
def make_cache_book(args, model, layer_books, observer_hit_rates):
    return {
        ...
        "config": {
            ...
            "seacache_threshold": args.seacache_thresh,
            "power_exp": 2.0,
            "filter_dims": [-2, -3],
            "scheduler_mode": "flow",
            "norm_mode": "mean",
        ...
        "calibration_diagnostics": {
            "observer_step_hit_rates": observer_hit_rates,
        },
...
def validate_cache_book(book, args, model):
    ...
    if config.get("seacache_threshold") != args.seacache_thresh:
        raise ValueError("SeaCache threshold mismatch")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Standalone FLUX SeaCache + Finegrained Cache sampler"
    )
    ...
    # Native FLUX-28 hybrid setting; SeaCache's step threshold stays at 0.30.
    ...
    add_preset_argument(parser, "flux_seacache_hybrid", tiers=("hybrid",))
    ...
    args = apply_preset(args, "flux_seacache_hybrid", {
...
def main():
    ...
    if args.finegrained_calibration:
        ...
        with torch.inference_mode():
            layer_books, observer_hit_rates = calibrate(
                pipe, model, calibration_prompts, args
            )
        book = make_cache_book(args, model, layer_books, observer_hit_rates)
