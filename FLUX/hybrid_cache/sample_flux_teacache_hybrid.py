...
TEACACHE_COEFFICIENTS = np.asarray(
    [
        4.98651651e02,
        -2.83781631e02,
        5.58554382e01,
...
class TeaCachePolicy:
    ...
    def decide(
        ...
        if (
            ...
            rescaled = np.poly1d(TEACACHE_COEFFICIENTS)(
                float(relative_l1.detach().cpu().item())
            )
            self.accumulated_distance += float(rescaled)
...
class HybridFluxTransformer(nn.Module):
    def __init__(self, base_model: nn.Module, num_steps: int):
        ...
        self.step_policy: Optional[TeaCachePolicy] = None
        self.reset_runtime()
...
def calibrate(pipe, model, prompts, args):
    print("Calibration pass 1/2: raw layer scores + TeaCache observer")
    raw_runs = []
    ...
    for prompt in tqdm(
        ...
        model.step_policy = TeaCachePolicy(
            args.num_inference_steps, args.teacache_thresh
        )
...
def default_cache_name(args):
    return (
        f"cache_book_hybrid_teacache_steps{args.num_inference_steps}"
        f"_ns{fmt(args.nonskip_rate)}_teath{fmt(args.teacache_thresh)}"
...
def make_cache_book(args, model, layer_books, observer_hit_rates):
    return {
        ...
        "config": {
            ...
            "teacache_threshold": args.teacache_thresh,
            "teacache_coefficients": TEACACHE_COEFFICIENTS.tolist(),
...
def validate_cache_book(book, args, model):
    ...
    if config.get("teacache_threshold") != args.teacache_thresh:
        raise ValueError("TeaCache threshold mismatch")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Standalone FLUX TeaCache + Finegrained Cache sampler"
    )
    ...
    # TeaCache external default remains thresh=0.60.
    ...
    parser.add_argument("--teacache-thresh", type=float, default=0.6)
    parser.add_argument(
