"""HunyuanVideo-1.5 Finegrained Cache layer-only sampler.
...
class FeatureChangeAnalyzer:
    ...
    def __init__(
        ...
        if correction_mode and module_books is None:
...
def _cache_filename(args, version, width, height, steps):
    ...
    return (
        f"cache_book_layer_{version}_{width}x{height}"
...
def generate(args):
    ...
    if args.invardiff_calibration:
        ...
        del raw_modules
        corrected_modules = _collect_stage(
            run_once,
            pipe.transformer,
            args,
            steps,
