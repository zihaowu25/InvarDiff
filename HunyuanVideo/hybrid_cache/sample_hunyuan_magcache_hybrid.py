...
SOURCE_COMMIT = "47bdd2a"
HUNYUAN_COMMIT = "60783e7"
OFFICIAL_MAG_RATIOS_20 = [1.0] * 2 + [
    1.01172, 1.00293, 0.9873, 1.0127, 1.01465, 0.98926, 0.99805,
    1.00098, 0.99512, 0.99365, 0.99512, 0.99561, 0.99316, 0.99365,
    0.99365, 0.99365, 0.99316, 0.99316, 0.99268, 0.99268, 0.9917,
    0.9917, 0.99023, 0.99023, 0.98828, 0.98877, 0.98633, 0.98633,
    0.9834, 0.9834, 0.97949, 0.97998, 0.97363, 0.97363, 0.96436,
    0.96436, 0.94824, 0.94873,
]
OFFICIAL_MAG_RATIOS_40 = [1.0] * 2 + [
    1.04199, 1.01953, 1.01172, 1.01855, 1.00293, 1.00586, 1.00098,
    1.00195, 1.0, 1.00098, 0.99805, 0.99902, 0.99854, 0.99805,
    0.99658, 0.99756, 0.99707, 0.99512, 0.99609, 0.99609, 0.99658,
    0.99658, 0.99658, 0.99805, 0.99658, 0.99707, 0.99561, 0.99561,
    0.99561, 0.99658, 0.99658, 0.99658, 0.99609, 0.99658, 0.99512,
    0.99561, 0.99463, 0.99512, 0.99463, 0.99512, 0.99463, 0.99512,
    0.99365, 0.99414, 0.99365, 0.99365, 0.99219, 0.99219, 0.99219,
    0.99268, 0.99072, 0.99121, 0.99072, 0.99072, 0.98926, 0.98975,
    0.9873, 0.98779, 0.98535, 0.98584, 0.9834, 0.9834, 0.97998,
    0.97998, 0.97607, 0.97607, 0.96973, 0.96973, 0.96338, 0.96338,
    0.9502, 0.9502, 0.93066, 0.93066, 0.896, 0.896, 0.81787, 0.81836,
]
...
def _global_mean(values: torch.Tensor) -> float:
    pair = torch.stack((values.detach().float().sum(), torch.tensor(
        float(values.numel()), device=values.device, dtype=torch.float32
    )))
    pair = _reduce_sum(pair)
    return float((pair[0] / pair[1].clamp_min(1.0)).item())


class MagCalibrationCollector:
    """Collect token-wise residual L2 magnitude ratios for both CFG halves."""

    def __init__(self, num_steps: int, do_cfg: bool):
        self.num_steps = int(num_steps)
        self.do_cfg = bool(do_cfg)
        self.previous_norm = [None, None]
        self.ratios = [[1.0] * num_steps for _ in range(2)]

    def update(self, step_idx: int, residual: torch.Tensor):
        chunks = residual.chunk(2, dim=0) if self.do_cfg else (residual,)
        # Native batch order is [unconditional, conditional].
        for native_slot, chunk in enumerate(chunks):
            norm = chunk.detach().float().norm(p=2, dim=-1)
            previous = self.previous_norm[native_slot]
            if previous is not None:
                ratio = _global_mean(norm / previous.clamp_min(1e-8))
                self.ratios[native_slot][step_idx] = round(ratio, 5)
            self.previous_norm[native_slot] = norm

    def semantic_interleaved(self):
        if self.do_cfg:
            # Official order is [conditional, unconditional].
            return [
                self.ratios[slot][step]
                for step in range(self.num_steps) for slot in (1, 0)
            ]
        return [
            value for step in range(self.num_steps)
            for value in (self.ratios[0][step], self.ratios[0][step])
        ]


class MagCachePolicy:
    """Official HunyuanVideo-1.5 accumulator with semantic CFG slots."""

    def __init__(self, ratios, num_steps, threshold, max_skip, retention, enabled=True):
        if len(ratios) != num_steps * 2:
            raise ValueError("MagCache ratios must contain two values per step")
        self.ratios = [float(value) for value in ratios]
        self.num_steps = int(num_steps)
    ...
    def decide_semantic(self, step_idx: int, semantic_slot: int) -> bool:
        if not self.enabled or step_idx < int(self.num_steps * self.retention):
            return False
        ratio = self.ratios[2 * step_idx + semantic_slot]
        self.accumulated_ratio[semantic_slot] *= ratio
        self.accumulated_steps[semantic_slot] += 1
        self.accumulated_error[semantic_slot] += abs(
            1.0 - self.accumulated_ratio[semantic_slot]
        )
        skip = (
            self.accumulated_error[semantic_slot] < self.threshold
            and self.accumulated_steps[semantic_slot] <= self.max_skip
        )
        if not skip:
            self.accumulated_ratio[semantic_slot] = 1.0
            self.accumulated_error[semantic_slot] = 0.0
            self.accumulated_steps[semantic_slot] = 0
        return skip

    def decide_native(self, step_idx: int, do_cfg: bool):
        if not do_cfg:
            return [self.decide_semantic(step_idx, 0)]
        conditional = self.decide_semantic(step_idx, 0)
        unconditional = self.decide_semantic(step_idx, 1)
        return [unconditional, conditional]


def _derive_masks(ratios, num_steps, args, do_cfg):
    policy = MagCachePolicy(
        ratios, num_steps, args.magcache_thresh, args.magcache_K,
        args.retention_ratio, enabled=not args.disable_step_cache,
    )
    masks = [[False] * num_steps for _ in range(2)]
    for step in range(num_steps):
        decisions = policy.decide_native(step, do_cfg)
        if do_cfg:
            masks[0][step], masks[1][step] = decisions
        else:
            masks[0][step] = decisions[0]
    return masks
...
def _reset_runtime(model):
    ...
    model.finegrained_previous_step_cached = [False, False]
    model.finegrained_force_refresh = False
    model.finegrained_active_slots = (0, 1) if model.finegrained_do_cfg else (0,)
...
def finegrained_transformer_forward(
    ...
    do_cfg = self.finegrained_do_cfg
    native_slots = (0, 1) if do_cfg else (0,)
    policy = self.finegrained_step_policy
    decisions = (
        policy.decide_native(step_idx, do_cfg)
        if policy is not None
        else [False] * len(native_slots)
    )
    if self.finegrained_observer_masks is not None:
        for slot in native_slots:
            self.finegrained_observer_masks[slot][step_idx] = bool(decisions[slot])

    batch_per_slot = img.shape[0] // len(native_slots)
    if batch_per_slot * len(native_slots) != img.shape[0]:
        raise ValueError("CFG batch cannot be divided into equal slots")

    def select_batch(value, slot):
        if value is None or not torch.is_tensor(value):
            return value
        if value.shape[0] != img.shape[0]:
            return value
        begin = slot * batch_per_slot
        return value[begin : begin + batch_per_slot]

    def run_blocks(img_arg, txt_arg, vec_arg, mask_arg, slots, force_refresh):
        self.finegrained_active_slots = tuple(slots)
        self.finegrained_force_refresh = bool(force_refresh)
        local_features = [] if output_features else None
        for index, block in enumerate(self.double_blocks):
            ...
            img_arg, txt_arg = block(
                img_arg, txt_arg, vec_arg, freqs_cis, mask_arg,
                self.attn_param, force_full, index,
            )
        txt_len = txt_arg.shape[1]
        img_len = img_arg.shape[1]
        x_arg = torch.cat((img_arg, txt_arg), dim=1)
        for index, block in enumerate(self.single_blocks):
            ...
            x_arg = block(
                x_arg, vec_arg, txt_len, (freqs_cos, freqs_sin), mask_arg,
                self.attn_param, force_full,
            )
            if output_features and index % output_features_stride == 0:
                local_features.append(x_arg[:, :img_len])
        self.finegrained_force_refresh = False
        return x_arg[:, :img_len], local_features
    ...
    can_run_full = (
        not any(actual_hits)
        and len(set(self.finegrained_previous_step_cached[slot] for slot in native_slots)) == 1
    )
    if can_run_full:
        img, features_list = run_blocks(
            img, txt, vec, text_mask, native_slots,
            self.finegrained_previous_step_cached[native_slots[0]],
        )
        residual = img - img_before_blocks
        residual_chunks = residual.chunk(len(native_slots), dim=0)
        if self.finegrained_mag_collector is not None:
            self.finegrained_mag_collector.update(step_idx, residual)
        if self.finegrained_execute_step_cache:
            for slot, chunk in zip(native_slots, residual_chunks):
                self.finegrained_runtime_cache.put(
                    ("step", -1, slot), chunk, has_future=True
                )
        for slot in native_slots:
            self.finegrained_computed_steps[slot] += 1
    else:
        outputs = []
        slot_features = []
        for slot in native_slots:
            img_slot = select_batch(img_before_blocks, slot)
            if actual_hits[slot]:
                residual = self.finegrained_runtime_cache.get(
                    ("step", -1, slot), img.device, last_use=False
                )
                outputs.append(img_slot + residual)
                self.finegrained_step_hits[slot] += 1
                continue
            txt_slot = select_batch(txt, slot)
            vec_slot = select_batch(vec, slot)
            mask_slot = select_batch(text_mask, slot)
            out_slot, local_features = run_blocks(
                img_slot, txt_slot, vec_slot, mask_slot, (slot,),
                self.finegrained_previous_step_cached[slot],
            )
            outputs.append(out_slot)
            if output_features:
                slot_features.append(local_features)
            self.finegrained_runtime_cache.put(
                ("step", -1, slot), out_slot - img_slot, has_future=True
            )
            self.finegrained_computed_steps[slot] += 1
        img = outputs[0] if len(outputs) == 1 else torch.cat(outputs, dim=0)
        if output_features:
            features_list = [
                torch.cat([values[index] for values in slot_features], dim=0)
                for index in range(len(slot_features[0]))
            ]
    for slot in native_slots:
        self.finegrained_previous_step_cached[slot] = actual_hits[slot]
    self.finegrained_active_slots = native_slots
...
def _init_runtime(
    model, args, num_steps, do_cfg, books=None, analyzer=None, use=False,
    step_policy=None, execute_step_cache=False, observer_masks=None,
    mag_collector=None,
):
    ...
    model.finegrained_observer_masks = observer_masks
    model.finegrained_mag_collector = mag_collector
    model.finegrained_previous_step_cached = [False, False]
    model.finegrained_force_refresh = False
    model.finegrained_active_slots = (0, 1) if do_cfg else (0,)
    model.finegrained_step_hits = [0, 0]
    model.finegrained_computed_steps = [0, 0]
...
def _release_stage(model, analyzer):
    ...
    model.finegrained_mag_collector = None
    gc.collect()
    if torch.cuda.is_available():
...
def _collect_stage(
    run_once, model, args, num_steps, depths, do_cfg, description,
    books=None, step_masks=None, collector=None,
):
    active = tuple(
        name for name, value in _thresholds(args).items() if value > 0
    )
    analyzer = FeatureChangeAnalyzer(
        num_steps, depths, active, args.calibration_feature_device, do_cfg,
        correction_mode=books is not None,
        step_mask=step_masks[1] if step_masks is not None else None,
        module_books=books,
    )
    _init_runtime(
        model, args, num_steps, do_cfg, analyzer=analyzer, use=False,
        step_policy=None, execute_step_cache=False, mag_collector=collector,
    )
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    ...
    scores = {
        name: value.clone() for name, value in analyzer.module_scores.items()
    }
    _release_stage(model, analyzer)
    return scores, peak


def _official_compatible(args, task, steps):
    return (
        task == "t2v"
        and steps in (20, 40)
        and not args.cfg_distilled
        and not args.enable_step_distill
        and not args.sparse_attn
    )


def _official_ratios(steps):
    if steps == 20:
        return list(OFFICIAL_MAG_RATIOS_20)
    if steps == 40:
        return list(OFFICIAL_MAG_RATIOS_40)
    raise ValueError("Official HunyuanVideo-1.5 ratios cover only 20/40 steps")


def _resolve_ratio_source(args, task, steps):
    compatible = _official_compatible(args, task, steps)
    if args.magcache_ratio_source == "official":
        if not compatible:
            raise ValueError(
                "Official MagCache ratios support only standard T2V 20/40-step "
                "models; use joint calibration for this configuration"
            )
        return "official_hunyuanvideo15"
    if args.magcache_ratio_source == "calibrated":
        return "joint_calibration"
    return "official_hunyuanvideo15" if compatible else "joint_calibration"


def _cache_config(args, version, task, width, height, steps, depths):
    return {
        "transformer_version": version, "task": task, "width": width,
        "height": height, "frames": args.video_length, "steps": steps,
        "nonskip": args.nonskip_rate,
        "module_thresholds": _thresholds(args), "seed": args.seed,
        ...
        "ratio_source": args.magcache_resolved_source,
        "step_policy_source": "official_hunyuanvideo15",
...
def _cache_filename(args, version, width, height, steps):
    ...
    return (
        f"cache_book_hybrid_magcache_{version}_{width}x{height}"
        f"_f{args.video_length}_steps{steps}_ns{fmt(args.nonskip_rate)}"
...
def _save_books(path, books, config, ratios, masks, peaks):
    payload = {
        "cache_scope": CACHE_SCOPE, "policy": POLICY_VARIANT,
        "step_policy": STEP_POLICY, "rate_method": RATE_METHOD,
        "config": config,
        "finegrained_cache": {"module_cache_book": books},
        "step_policy_artifacts": {
            "ratio_order": ["conditional", "unconditional"],
            "ratios": {
                "conditional": ratios[0::2],
                "unconditional": ratios[1::2],
            },
            "step_masks": {
                "unconditional": masks[0], "conditional": masks[1],
            },
        },
...
def _load_books(path, expected, steps, depths):
    ...
    if (
        ...
        raise ValueError("Legacy or incompatible MagCache hybrid Cache Book")
    ...
    required = (
        ...
        "double_blocks",
        "single_blocks", "magcache_thresh", "magcache_K",
        "retention_ratio", "ratio_source", "source_commit",
        "hunyuanvideo_1_5_commit",
    )
    for key in required:
    ...
    books = payload.get("finegrained_cache", {}).get("module_cache_book")
    if not isinstance(books, dict) or set(books) != set(MODULES):
        raise ValueError("Invalid module_cache_book keys")
    for name in MODULES:
        if len(books[name]) != steps or any(
            not isinstance(row, list)
            or len(row) != depths[name]
    ...
    ratio_data = artifacts.get("ratios", {})
    cond = ratio_data.get("conditional")
    uncond = ratio_data.get("unconditional")
    mask_data = artifacts.get("step_masks", {})
    masks = [mask_data.get("unconditional"), mask_data.get("conditional")]
    if not all(isinstance(values, list) and len(values) == steps for values in (cond, uncond, *masks)):
        raise ValueError("Invalid MagCache ratio or mask artifacts")
    ratios = [value for pair in zip(cond, uncond) for value in pair]
    return books, ratios, masks
...
def generate(args):
    ...
    args.magcache_resolved_source = _resolve_ratio_source(args, task, steps)
    if (
        args.magcache_resolved_source == "joint_calibration"
        and not args.finegrained_calibration
        and not args.use_finegrained_cache
        and not args.disable_step_cache
    ):
        raise ValueError(
            "This configuration has no official MagCache ratio table. Run "
            "joint calibration and then load its hybrid Cache Book."
        )
    ...
    books = None
    ratios = (
        _official_ratios(steps)
        if args.magcache_resolved_source == "official_hunyuanvideo15"
        else None
    )
    masks = None
    ...
    if args.finegrained_calibration:
        collector = MagCalibrationCollector(steps, do_cfg)
        raw_modules, raw_peak = _collect_stage(
            run_once, pipe.transformer, args, steps, depths, do_cfg,
            "joint raw MagCache and layer calibration",
            collector=collector,
        )
        if args.magcache_resolved_source == "joint_calibration":
            ratios = collector.semantic_interleaved()
        masks = _derive_masks(ratios, steps, args, do_cfg)
        provisional = _books_from_scores(
            raw_modules, args.nonskip_rate, _thresholds(args)
        )
        del raw_modules, collector
        corrected_modules, correction_peak = _collect_stage(
            run_once, pipe.transformer, args, steps, depths, do_cfg,
            "MagCache-aware correction calibration",
            books=provisional, step_masks=masks,
        )
    ...
    if enabled:
        _init_runtime(
            ...
            step_policy=MagCachePolicy(
                ratios, steps, args.magcache_thresh, args.magcache_K,
                args.retention_ratio, enabled=not args.disable_step_cache,
    ...
    if enabled:
        ...
        labels = ("unconditional", "conditional") if do_cfg else ("model",)
        for slot, label in enumerate(labels):
            hits = pipe.transformer.finegrained_step_hits[slot]
            rank0_log(
                f"MagCache {label} step hits: {hits}/{total} "
                f"({hits / total:.2%})"
            )
        ...
        computed = max(1, sum(
            pipe.transformer.finegrained_computed_steps[: len(labels)]
        ))
        layer_hits = pipe.transformer.finegrained_layer_hits
        rank0_log(
            f"Computed-branch layer hits: {layer_hits}/"
            f"{computed * module_count} "
            f"({layer_hits / (computed * module_count):.2%})"
        )
        total_branches = total * len(labels)
        effective = (
            sum(pipe.transformer.finegrained_step_hits[: len(labels)])
            * module_count + layer_hits
        ) / (total_branches * module_count)
...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "HunyuanVideo-1.5 MagCache + Finegrained Cache hybrid sampler"
        )
    )
    ...
    parser.add_argument("--magcache_thresh", type=float, default=0.03)
    parser.add_argument("--magcache_K", type=int, default=2)
    parser.add_argument("--retention_ratio", type=float, default=0.25)
    parser.add_argument(
        "--magcache_ratio_source",
