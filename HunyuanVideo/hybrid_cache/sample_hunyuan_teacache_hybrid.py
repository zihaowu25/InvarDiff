...
class TeaCachePolicy:
    """Official HunyuanVideo TeaCache accumulator for one CFG batch."""

    def __init__(
        self, num_steps: int, threshold: float, enabled: bool = True,
        feature_device: str = "gpu",
    ):
    ...
    def decide(self, step_idx: int, modulated_input: torch.Tensor) -> bool:
        ...
        if self.previous_modulated_input is None or first_or_last:
            ...
            relative = _relative_l1(modulated_input, self.previous_modulated_input)
            self.accumulated_distance += float(
                np.polyval(TEACACHE_COEFFICIENTS, relative)
            )
            should_skip = self.accumulated_distance < self.threshold
            if not should_skip:
                self.accumulated_distance = 0.0
        self.previous_modulated_input = _history_tensor(
            modulated_input, self.feature_device
        )
...
def finegrained_transformer_forward(
    ...
    if policy is not None:
        ...
        would_skip = policy.decide(step_idx, modulated_input)
...
def _collect_stage(
    ...
    _init_runtime(
        ...
        analyzer=analyzer,
        use=False,
        step_policy=TeaCachePolicy(
            num_steps, args.teacache_thresh,
...
def _cache_config(args, version, task, width, height, steps, depths):
    return {
        ...
        "teacache_coefficients": list(TEACACHE_COEFFICIENTS),
...
def _cache_filename(args, version, width, height, steps):
    ...
    return (
        f"cache_book_hybrid_teacache_{version}_{width}x{height}"
...
def _load_books(path, expected, steps, depths):
    ...
    required = (
        ...
        "double_blocks",
        "single_blocks", "teacache_thresh", "teacache_coefficients",
...
def generate(args):
    ...
    if args.finegrained_calibration:
        ...
        corrected_modules, _, correction_peak = _collect_stage(
            run_once, pipe.transformer, args, steps, depths, do_cfg,
            "TeaCache-aware correction calibration",
    ...
    if enabled:
        ...
        rank0_log(
            f"TeaCache step hits: {step_hits}/{total} ({step_hits / total:.2%})"
        )
        rank0_log(
...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "HunyuanVideo-1.5 TeaCache + Finegrained Cache hybrid sampler"
        )
    )
    ...
    parser.add_argument("--teacache_thresh", type=float, default=0.15)
