...
class FeatureChangeAnalyzer:
    """GPU-resident analyzer for Wan step and layer features with compressed state."""
    ...
    def __init__(
        ...
        if correction_mode and (
            step_cache_state is None or module_cache_state is None
        ):
            raise ValueError(
                "Correction mode requires provisional step and module cache books."
            )
    ...
    def update_step(self, runtime_step_idx: int, hidden_states: torch.Tensor):
        """Update only from the conditional (even runtime-index) CFG branch."""
        if runtime_step_idx % 2 != 0:
            return
        ...
        should_refresh, self._step_state = self._refresh_state(
            bool(self.step_cache_state[score_idx]),
            self._step_state,
        )
        if should_refresh:
            self._rotate_step(hidden_states, current_norm)
...
def _books_from_scores(
    ...
    if not 0.0 <= step_thres <= 1.0:
        raise ValueError(f"step_thres must be in [0, 1], got {step_thres}.")
    ...
    if step_thres > 0.0:
        step_cache_bool[1:-1] = _compute_quantile_mask(
            step_scores[1:-1],
            step_thres,
        )
...
def _load_cache_books(
    ...
    step_cache_bool = payload.get("step_cache_book")
    module_cache_book = payload.get("module_cache_book", {})
    if not isinstance(step_cache_bool, list) or len(step_cache_bool) != args.sample_steps:
        raise ValueError(
            "Invalid step cache book length: expected "
            f"{args.sample_steps}, got "
            f"{len(step_cache_bool) if isinstance(step_cache_bool, list) else 'non-list'}."
        )
    if any(type(value) is not bool for value in step_cache_bool):
        raise ValueError("Step cache book must contain JSON booleans only.")
...
def _build_default_cache_book_filename(args) -> str:
    ...
    return (
        f"cache_book_stplayer_{args.task}_{size}_f{args.frame_num}"
...
def _invardiff_core_forward(self, x, e, kwargs, grid_sizes):
    ...
    collect_conditional = analyzer is not None and slot == 0
    ...
    if use_step_cache:
        ...
        for layer_idx, block in enumerate(self.blocks):
            ...
            if self.invardiff_calib_running:
                layer_cache = {k: None for k in WAN_CACHE_MODULES}
            else:
                layer_cache = {
            ...
            for mod_name in WAN_CACHE_MODULES:
                ...
                if not self.invardiff_calib_running:
                    self.module_feature_cache[mod_name][slot][layer_idx] = (
                        outs[mod_name].detach()
                    )
    ...
    if analyzer is not None:
        analyzer.update_step(step_idx, x)
...
def _collect_calibration_stage(
    ...
    """Collect step and all active-module scores in one generation run."""
    correction_mode = step_cache_state is not None
    analyzer = FeatureChangeAnalyzer(
        policy_steps,
        num_layers,
        correction_mode=correction_mode,
        step_cache_state=step_cache_state,
    ...
    step_scores = analyzer.step_scores.clone()
    module_scores = {
        key: value.clone() for key, value in analyzer.module_scores.items()
    }
    _release_calibration_state(model, analyzer)
    del analyzer
    return step_scores, module_scores
...
def _calibrate_invardiff(run_once, model, args):
    ...
    # Phase 1: one full run collects step and all active-module statistics.
    raw_step_scores, raw_module_scores = _collect_calibration_stage(
        run_once,
        model,
        policy_steps,
        runtime_steps,
    ...
    del raw_step_scores, raw_module_scores

    # Phase 2: correction pass with cache-state-aware baseline updates.
    corrected_step_scores, corrected_module_scores = _collect_calibration_stage(
        run_once,
        model,
        policy_steps,
        runtime_steps,
    ...
    del corrected_step_scores, corrected_module_scores
...
def _prepare_invardiff_books(model, args):
    ...
    if args.use_invardiff and not args.invardiff_calibration:
        cache_file = args.cache_book_file or _build_default_cache_book_filename(args)
        cache_path = os.path.join(args.cache_book_path, cache_file)
        step_cache_bool, module_cache_book = _load_cache_books(
            cache_path,
            args,
            len(model.blocks),
        )
